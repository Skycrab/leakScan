    ...
    $(".start_task").bind('click',function(){
        ...
        $.post("/task",
                ...
                function(data){
                    d = analyse(data);
                    if(d!== false){
                        node.find("img").attr('src','/static/img/running.gif');
                    }
                }
            );
    });
...
//单个任务绑定继续扫描
function task_bind_continue_start(){
    $(".continue_task").bind('click',function(){
        var node = $(".open").parent().parent();
        var task_id = node.children(".hide").html();
        $.post("/task",
                {
...
//单个任务绑定重新扫描
function task_bind_restart(){
    $(".restart_task").bind('click',function(){
        var node = $(".open").parent().parent();
        var task_id = node.children(".hide").html();
        $.post("/task",
                {
                ...
                function(data){
                    ...
                    if(d!== false){
                        node.find("img").attr('src','/static/img/running.gif');
                        node.find(".high_risk").attr("style","width:0%");
                        node.find(".middle_risk").attr("style","width:0%");
                        node.find(".low_risk").attr("style","width:0%");
                        node.find(".no_risk").attr("style","width:100%");
                        node.find(".no_risk_num").html("0(漏洞)");
                    }
                }
            );
    });
    ...
    $(".stop_task").bind('click',function(){
        ...
        $.post("/task",
                ...
                function(data){
                    d = analyse(data);
                    if(d!== false){
                        node.find("img").attr("src","/static/img/stop.png");
                    }
                }
            );
    });
                    ...
                    var basic_info = function(){
                        $.get('/detail?action=basic&task_id=' + task_id, function (data) {
                            ...
                            if(d !== false){
                                ...
                                if(d.task_status != 1){ //任务不在运行，清空定时器
                                    clearInterval(timer);
                                }
                            }
                        });
...
$(function(){
    ...
    //刷新按钮事件绑定
    $("#refresh_task").bind("click",function(){
        $.post("/task",
            {
                'action': 'refresh'
            },
